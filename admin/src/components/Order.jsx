...
order._id===orderId?{...order,status:newStatus}:order
  ...
  if(loading){
  return (
    <section className="relative w-full min-h-screen  bg-linear-to-r from-green-700 via-green-600 to-green-500
...
return(
  <section className="relative w-full min-h-screen md:ml-64  bg-linear-to-r from-green-700 via-green-600 to-green-500
     text-white py-24 px-6 sm:px-10">
{orders.length===0?(
...
{orders.map((order)=>{
  ...
  return(
    <div key={order._id} className="bg-white/10 backdrop-blur-md
...
Order ID:{order._id.slice(-6).toUpperCase()}
  ...
  <p className="text-gray-200 mb-2 text-sm"><span className="font-semibold">العنوان</span>{" "}
  {order.address?`${order.address.name},${order.address.address},${order.address.city},${order.address.phone}`:"ليس موجود"}
  </p>
...
{item.image&&(
  <img src={`${url}/image/${item.image}`} className="w-10 h-10 object-cover rounded" />
)}
<p className="text-gray-200 text-sm">{item.name} x {item.quantity || 1}</p>
</div>
<p className="font-semibold text-gray-100 text-sm">${item.price * (item.quantity || 1)}</p>
  </div>
))}
  </div>
</div>
